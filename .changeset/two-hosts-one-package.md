...
Support opencode 2 alongside opencode 1 from a single package. The default export now carries both entrypoints (`tui` for opencode 1, `setup` for opencode 2), so the same version works on either host.
